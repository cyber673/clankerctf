...
3. Spawn the `ctf` subagent (Codex routes by agent `name`). In your prompt, ask Codex to spawn `ctf` and brief it with:
...
4. For multiple independent challenges, ask Codex to spawn one `ctf` subagent per challenge and consolidate results — Codex's orchestrator will wait for all of them and return a single response.
